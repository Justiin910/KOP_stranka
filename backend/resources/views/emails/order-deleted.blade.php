...
<body>
    <div class="container">
        <div class="header">
            <h1>{{ __('messages.email.order_deleted.header') }}</h1>
        </div>

        <div class="content">
            <p>{{ __('messages.email.order_deleted.greeting', ['name' => ($address['fullName'] ?? __('messages.email.order_confirmation.customer_fallback'))]) }}</p>
            <p>{{ __('messages.email.order_deleted.intro') }}</p>

            <div class="order-reference">
                {{ __('messages.email.order_confirmation.order_number') }}: {{ $order->reference }}
            </div>

            <h2 class="section-title">{{ __('messages.email.order_deleted.current_state_title') }}</h2>
            ...
            <table class="items-table">
                ...
                <tbody>
                    @foreach($items as $item)
                        ...
                        @endphp
                        <tr>
                            <td>{{ $productTitle ?? __('messages.email.order_deleted.product_fallback', ['id' => $productId]) }}</td>
                            <td>
            ...
            <p>{{ __('messages.email.order_deleted.footer_notice') }}</p>
        </div>
