<template>
	<div class="min-h-screen bg-white dark:bg-gray-900">
		<div class="max-w-7xl mx-auto px-6 py-12">
					...
					<div v-for="(step, index) in steps" :key="index" class="flex items-center">
						<div class="flex flex-col items-center">
							<div 
								...
								:class="step.active ? 'bg-blue-600 text-white' : 'bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400'"
							>
								{{ index + 1 }}
							</div>
							<span 
								class="mt-2 text-sm font-medium"
								:class="step.active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'"
							>
								{{ step.label }}
							</span>
						</div>
						<div 
							...
							:class="steps[index + 1].active ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'"
						></div>
					</div>
				</div>
			</div>
			...
			<!-- Empty Cart -->
			<div v-if="cartItems.length === 0" class="bg-gray-50 dark:bg-gray-800 rounded-lg p-8 text-center">
			...
			<div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					>
						<div class="flex gap-4">
							...
							<div class="text-right">
								<p class="text-xl font-bold text-gray-900 dark:text-white">{{ formatPrice(item.price * item.quantity) }} €</p>
								<p v-if="item.quantity > 1" class="text-sm text-gray-500 dark:text-gray-400">{{ formatPrice(item.price) }} € / ks</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="lg:col-span-1">
					<div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700 sticky top-6">
						<h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Súhrn objednávky</h2>
...
export default {
	data() {
		return {
			...
			cartItems: [
				// Príklad položiek - neskôr nahradíš skutočnými dátami
				{
					id: 1,
					name: 'iPhone 15 Pro',
